 ...
 */
public class TestRecordDeserializer<T> implements Deserializer<TestRecord> {
     ...
     */
    @Override
    public TestRecord deserialize(final String topic, final byte[] bytes) {
        if (bytes == null) return null;

        TestRecord data;
        try {
            data = objectMapper.readValue(bytes, TestRecord.class);
